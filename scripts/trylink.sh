...
if test "$CONFIG_BUILD_LIBBUSYBOX" = y; then
    ...
    $STRIP -s --remove-section=.note  $EXE -o "$sharedlib_dir/libbusybox.so.$BB_VER"
...
if test "$CONFIG_FEATURE_SHARED_BUSYBOX" = y; then
    ...
    $STRIP -s --remove-section=.note  $EXE -o "$sharedlib_dir/busybox"
...
" >"$sharedlib_dir/applet.c"
	...
	$STRIP -s --remove-section=.note  $EXE
